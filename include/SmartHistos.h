...
#include "TVectorT.h"
#include "TColor.h"  // Make sure to include ROOT's TColor header
		...
		void Fill(const bool debug = 0) {
			...
			void era_and_prelim_lat_(int year, double xmin, double xmax, double ymin, double ymax, bool debug = 0, bool in=0) {
				...
				if (app) {
					...
					if (app==2) text = "CMS #scale[0.7]{#font[52]{}}";
					if (app==3) text = "CMS";
			...
			void multidraw_with_legend_(size_t skip, std::vector<TH1D*>& hvec, std::vector<std::string> pf, std::string colz,
					std::string legtitle="", float x1=0.15, float y2=0.9, bool debug = true) {
				...
				for (size_t i=skip; i<hvec.size() ; ++i) if (hvec[i]->GetEntries()>0) {
					std::cout << "  " << i << ": " << hvec[i]->GetName() << std::endl;
					...
					if (stack_) {
						...
						std::string __nm = hvec[i]->GetName();
						bool is_data   = (i==0) || (__nm.find("Data") != std::string::npos);
						bool is_signal = (__nm.find("T5")   != std::string::npos) ||
							(__nm.find("T6")   != std::string::npos) || (__nm.find("R2bb")   != std::string::npos) ||
							(__nm.find("TChi") != std::string::npos) || (__nm.find("R5")   != std::string::npos) ||
							(__nm.find("_sig") != std::string::npos);
						if (is_data || is_signal) {
							if (i==0) {
								...
								//legentries.push_back(new TLegendEntry(hvec[i], "kBlack", "L"));
								n_nonstack_drawn++;
				...
				if (stack_) {
					...
					//}
					for (int i=(int)hvec.size()-1; i>=0; --i) {
						...
						bool is_signal = (__nm.find("T5")   != std::string::npos) ||
							...
							(__nm.find("TChi") != std::string::npos) || (__nm.find("R5")   != std::string::npos) ||
							(__nm.find("_sig") != std::string::npos);
						if (i!=0 && is_signal) hvec[i]->Draw("SAMEHIST");
					}
					// finally draw data on top
					if ((int)hvec.size()>0) hvec[0]->Draw("SAMEPE0");
				...
				// Legend
				//float textsize = ratio_ ? 0.188 : 0.40;
				//float x2 = x1 + 0.2 * (1+(twocol_>0))*(1+addint_*0.5);
				//float y1 = y2 - nrow * (textsize*1.4); // 0.1 + textsize + 0.1 (fEntrySeparation = 0.1)
				float x2 = 0.8;
				float y1 = 0.5;
				float textsize = 0.4;
				...
				if (twocol_) {
					...
					//size_t nrow1 = 5, nrow2 = 5;
					//cout << legentries.size() << endl;
					...
					for (size_t i=0; i<legentries.size(); ++i) {
						...
						if(i==2 || (i%2 == 0 && i>3))  leg->AddEntry((TObject*)0,"","");
					}
					//int value = 5;
					//nrow1 = (legentries.size()<9 ? 4 : 5);
					//nrow2 = (legentries.size()<9 ? 4 : 5);
					...
					//    leg->AddEntry(legentries[i]->GetObject(), legentries[i]->GetLabel(), legentries[i]->GetOption());
					//    if (addint_) {
					//			 TLegendEntry* l1 = leg->AddEntry(legentries[i+1]->GetObject(), legentries[i+1]->GetLabel(), legentries[i+1]->GetOption());
					//			 l1->SetTextColor(1);
					//		}
					//  } else {
					...
					//  // then right column(s)
					//  i = (1 + irow) * (1+addint_);
					//  if (i+addint_<legentries.size() && irow<value) {
					//    leg->AddEntry(legentries[i]->GetObject(), legentries[i]->GetLabel(), legentries[i]->GetOption());
					//    if (addint_) {
					//			 TLegendEntry* l1 = leg->AddEntry(legentries[i+1]->GetObject(), legentries[i+1]->GetLabel(), legentries[i+1]->GetOption());
					//			 l1->SetTextColor(1);
			...
			void add_stack_ratio_plot_(TCanvas*& c, double xmin, double xmax , bool remove=false, bool debug = 0) {
				...
				if (c->GetListOfPrimitives()->GetEntries()>2) {
					...
					TH1D* Data = nullptr;
					THStack* MCstack = nullptr;

					TList* prims = c->GetListOfPrimitives();
					if (!prims || prims->GetEntries() < 1) return;

					for (int ip = 0; ip < prims->GetEntries(); ++ip) {
						TObject* obj = prims->At(ip);
						if (!obj) continue;

						// First THStack we encounter
						if (!MCstack) MCstack = dynamic_cast<THStack*>(obj);

						// First histogram that looks like data
						if (!Data) {
							if (auto h1 = dynamic_cast<TH1D*>(obj)) {
								TString nm = h1->GetName();
								// Heuristic: name contains "Data" OR has markers (data points)
								if (nm.Contains("Data") || h1->GetMarkerStyle() > 0) Data = h1;
							}
						}
					}

					if (!MCstack || !MCstack->GetHists() || MCstack->GetHists()->GetEntries() == 0) {
						if (debug) std::cout << "[ratio] MCstack not found or empty; skip ratio.\n";
						return;
					}
					if (!Data) {
						if (debug) std::cout << "[ratio] Data histogram not found; skip ratio.\n";
						return;
					}
					if (debug) std::cout<<"ok1"<<std::endl;
					if (std::string(MCstack->GetTitle())!="0") {
						...
						leg->SetY1(leg_y2 - leg->GetNRows()*0.075);
						leg->SetY2(leg_y2);
						leg->SetX1(0.45);
						leg->SetX2(0.90);
						leg->Draw("SAME");
				...
				*/
				// --- REPLACE the whole block from "if (ok) {" to its closing brace ---

				if (ok) {
					if (debug) std::cout << "ok4" << std::endl;

					// Find the top pad robustly (do NOT assume At(0) is TPad)
					TPad* pad = nullptr;
					{
						TList* cl = c->GetListOfPrimitives();
						if (cl) {
							for (int ip=0; ip<cl->GetEntries(); ++ip) {
								if (auto p = dynamic_cast<TPad*>(cl->At(ip))) { pad = p; break; }
							}
						}
					}
					if (!pad) { if (debug) std::cout << "[ratio] no TPad found on canvas\n"; }
					else {
						if (debug) std::cout << "ok5" << std::endl;

						TList* plist = pad->GetListOfPrimitives();
						if (!plist) { if (debug) std::cout << "[ratio] pad primitives empty\n"; }
						else {
							// Scale factor for labels (same as before)
							const double scale = (y1 + y2) / y1;
							std::vector<TLatex*> topTexts;
							for (int i = 0; i < plist->GetEntries(); ++i) {
								if (auto lat = dynamic_cast<TLatex*>(plist->At(i))) {
									if (lat->GetY() > 0.80) topTexts.push_back(lat);
								}
							}

							// Sort by X to guess left(CMS) / right(era)
							std::sort(topTexts.begin(), topTexts.end(),
									[](const TLatex* a, const TLatex* b){ return a->GetX() < b->GetX(); });

							// Apply approval bits
							if (!topTexts.empty()) {
								if (approval_/10 > 0) {
									if (auto cmsLat = topTexts.front()) {
										cmsLat->SetTextSize(cmsLat->GetTextSize() * scale);
									}
								}
								if (approval_%10 > 0) {
									if (auto eraLat = topTexts.back()) {
										eraLat->SetTextSize(eraLat->GetTextSize() * scale);
									}
								}
							}
			...
			TCanvas* draw_roc_plot_(int debug = 0) {
				...
				float x2 = x1 + 0.3 * (1+(twocol_>0))*(1+addint_*0.3), y2 = y1 + nrow * (textsize*1.4);
				...
				leg->SetNColumns(ncol);
				if (twocol_) {
					size_t nrow1 = twocol_/11, nrow2 = twocol_%11;
					//size_t nrow1 = twocol_/5, nrow2 = twocol_%5;
					for (size_t irow=0, n = std::max(nrow1,nrow2); irow<n; ++irow) {
						// add left column(s) first
						size_t i = irow * (1+addint_);
						if (i+addint_<legentries.size() && irow<nrow1) {
