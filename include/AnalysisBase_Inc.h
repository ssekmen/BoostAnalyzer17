...
*/
        ...
        TGraphAsymmErrors* eff_trigger_muon;
        TGraphAsymmErrors* eff_trigger_photon;
        ...
        void AnalysisBase::init_syst_input() {
          ...
          // 1D Trigger efficiency
              ...
              TH1D* pass_ele  = getplot_TH1D("trigger_eff/INC/MET.root", "ele_pt_TrigNoMass_1", "trig01_ele");
              TH1D* total_ele = getplot_TH1D("trigger_eff/INC/MET.root", "ele_pt_TrigNoMass_0", "trig02_ele");
              eff_trigger_ele = new TGraphAsymmErrors(pass_ele, total_ele, "cl=0.683 b(1,1) mode");

              TH1D* pass_muon  = getplot_TH1D("trigger_eff/INC/MET.root", "mu_pt_TrigNoMass_1", "trig01_muon");
              TH1D* total_muon = getplot_TH1D("trigger_eff/INC/MET.root", "mu_pt_TrigNoMass_0", "trig02_muon");
              eff_trigger_muon = new TGraphAsymmErrors(pass_muon, total_muon, "cl=0.683 b(1,1) mode");

              TH1D* pass_photon  = getplot_TH1D("trigger_eff/INC/MET.root", "pho200_pt_TrigNoMass_1", "trig01_pho");
              TH1D* total_photon = getplot_TH1D("trigger_eff/INC/MET.root", "pho200_pt_TrigNoMass_0", "trig02_pho");
              eff_trigger_photon = new TGraphAsymmErrors(pass_photon, total_photon, "cl=0.683 b(1,1) mode");
	  ...
	  double w = 1;
          bool electron = false;
          bool muon = false;
          if(nMuVeto==0 && nEleTight==1 ) electron = true;
          if(nMuTight==1 && nEleVeto==0) muon = true;


          if (nJetAK8>0) {
              if (electron) {
                    geteff_AE(eff_trigger_ele, data.Electron[0].pt, eff, err_up, err_down);
                    w = get_syst_weight(eff, eff+err_up, eff-err_down, nSigmaTrigger);
                    return w;

              }else if(muon){
                    geteff_AE(eff_trigger_muon, data.Muon[0].pt, eff, err_up, err_down);
                    w = get_syst_weight(eff, eff+err_up, eff-err_down, nSigmaTrigger);
                    return w;

              }  else if(nPhotonSelect==1){   // PhotonjetsToInv region; Trigger Eff. weight for SinglePhoton triggers
                      geteff_AE(eff_trigger_photon, data.Photon[0].pt, eff, err_up, err_down);
                      w = get_syst_weight(eff, eff+err_up, eff-err_down, nSigmaTrigger);
                      return w;
                } else return w=1;
          } else return 0;
