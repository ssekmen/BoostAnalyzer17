...
pair<double, double> PurityFit(TH1D* chisoFake, TH1D* chisoPrompt, TH1D* chiso, string EBEE, string MRR2Bin){
	...
	CMS_lumi(C0, 2, 1);
	//C0->SetLogx();
	C0->SaveAs(Form("Test2017_1/Fraction_2017_Test_%s_%s.pdf", EBEE.c_str(), MRR2Bin.c_str()), "recreate");	
	
  TFile* f1 = new TFile("./Test2017_1/test.root", "update");
  C0->Write();
  result->Write();
  f1->Close();
  C0->Close();
...
pair<TH1D*, TH1D*> Purity2D(TFile* file, TString year){
	TH1::SetDefaultSumw2();
	//template - fake
	TH2D* htemp0_EB = (TH2D*)file->Get("MRR2NoPhoBins_vs_PhotonCHIso_preslectphoptREBin1/Data_FakePhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Barrel"); 	//Barrel
	TH2D* htemp0_EE = (TH2D*)file->Get("MRR2NoPhoBins_vs_PhotonCHIso_preslectphoptREBin1/Data_FakePhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Endcap"); 	//Endcap
	TH2D* htemp1_EB = (TH2D*)file->Get("MRR2NoPhoBins_vs_PhotonCHIso_preslectphoptREBin1/MC_FakePhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Barrel");	//Barrel
	TH2D* htemp1_EE = (TH2D*)file->Get("MRR2NoPhoBins_vs_PhotonCHIso_preslectphoptREBin1/MC_FakePhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Endcap");	//Barrel
	//template - prompt
	TH2D* htemp2_EB = (TH2D*)file->Get("MRR2NoPhoBins_vs_PhotonCHIso_preslectphoptREBin1/MC_PromptPhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Barrel");	//Barrel
	TH2D* htemp2_EE = (TH2D*)file->Get("MRR2NoPhoBins_vs_PhotonCHIso_preslectphoptREBin1/MC_PromptPhoton_CR_1PhoInv_Excl1Pho1M_"+year+"_Endcap");	//Endcap
	//fitting target
	TH2D* htemp3_EB = (TH2D*)file->Get("MRR2NoPhoBins_vs_PhotonCHIso_preslectphoptREBin1/Data_PhotonNoIso_CR_1PhoInv_Excl1Pho_"+year+"_Barrel");	//Barrel (Template)
	TH2D* htemp3_EE = (TH2D*)file->Get("MRR2NoPhoBins_vs_PhotonCHIso_preslectphoptREBin1/Data_PhotonNoIso_CR_1PhoInv_Excl1Pho_"+year+"_Endcap");	//Endcap (Template)
  ...
  cout << "end Fit" << endl;
	...
	leg->Draw("SAME");
	CMS_lumi(C1, 2, 1);
	C1->SaveAs(Form("Test2017_1/PurityResults.pdf"), "recreate");
...
// TGraphAsymmErrors *. -------------------------------------------------------
  ...
  c1->SaveAs(Form("Test2017_1/CorrectionFactor_fraction.pdf"), "recreate");
...
void results(){
	...
	//inputfile2 = TFile::Open("GJetsMC.root");
	inputfile2 = TFile::Open("run_20210505.root");
	inputfile3 = TFile::Open("run_20210505.root");

    TFile* f1  = new TFile("Test2017_1/CF_GJets2017.root","recreate");
	//f1->mkdir("purity");
	//f1->mkdir("fraction");
	//f1->mkdir("correction");
...
*/
  ...
  TGraphAsymmErrors* CorFac2017 = Correction(purity_MRR2_EB_2017, purity_MRR2_EE_2017, frac_EB, frac_EE, inputfile3, "2017");
  ...
  C1->SaveAs("Test2017_1/Purity_EE.pdf", "recreate");
  C1->Close();

  C2->cd();
  purity_MRR2_EB_2017->Draw("EP");
  C2->SaveAs("Test2017_1/Purity_EB.pdf", "recreate");
  C2->Close();
